---
name: empiricism
description: Authoring discipline for **new** tests — before writing any assertion, verify empirically what the SUT actually does (read the PHP, probe the live page, inspect rendered HTML, capture cookies/headers), and only then write the test against the observed behaviour. The shorthand: *"Fair point — I'm assuming. Let me verify empirically."* This skill walks you through that loop. It is for **adding new tests**, never for "fixing" intentionally-failing ones — the §9 gap tests and the documented cross-browser reds are red **by design** and stay red until CURA changes. Use whenever the user asks to add a test, write a scenario, cover a new flow, harden coverage, or capture a behaviour the suite doesn't yet exercise. Verify; do not theorise.
---
...
from a comment that says _"CURA does X"_ without a source citation.
...
The intentional reds:

| Test                                                                        | Why it's red                                | Reference                                     |
| --------------------------------------------------------------------------- | ------------------------------------------- | --------------------------------------------- |
| `Appointment - Past date booking accepted`                                  | CURA has no temporal validation             | FRD §9.7                                      |
| `Appointment - Server accepts empty date when client bypass applied`        | CURA has no server-side date validation     | FRD §9.1                                      |
| `Appointments - Duplicate booking (same facility, date, program)`           | CURA has no uniqueness constraint           | FRD §9.6                                      |
| `Appointments - Overlapping appointments (same date, different facilities)` | CURA has no geographic-conflict detection   | FRD §9.6                                      |
| `Journey - History ordered most-recent date first`                          | CURA renders history in submission order    | FRD §9.8 / §4.4                               |
...
If a new test you're about to author would assert the **opposite** of any of these (i.e. assert that CURA _does_ enforce the constraint, or that
Chrome _does not_ restore from BFcache), **stop**. That's not a new test — it's an attempt to flip a gap-test to green. The gap stays red until CURA
is fixed; the suite is _intentionally_ not silent about the defect.

Same rule for the empirical loop itself: if a probe surprises you with _"actually CURA does enforce X now"_, the right response is **not** "flip the
gap test to green." The right response is:

- Re-run the probe carefully (deterministic? on a clean browser? on the deployed app, not a fork?).
- If genuinely fixed, that's a finding worth a separate change: update `CURA_FRD.md` §9.x, update `IDENTIFIED_GAPS.md`, then — and only then — rewrite
  the relevant gap test as a regression-guard happy path.
...
For the example: the assertion rests on at least four assumptions about CURA's behaviour:

1. After a fresh login, navigating to `/history.php` lands you on a page (no redirect).
...
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Read the PHP source                | The claim is about a server-side decision (which template renders, whether a redirect fires, what query the page runs) | `gh api repos/katalon-studio/katalon-demo-cura/contents/<file>.php --jq '.content' \| base64 -d`                                                                                 |
...
| Drive the exact flow               | The claim is about an interaction's _outcome_ on the exact target                                                      | A throwaway probe that drives the exact locator on the exact screen with the exact wait condition and the exact action (`CLAUDE.md` → "A probe must exercise the exact target"). |

If two tools give the same answer, prefer the cheaper one. If they disagree (e.g. PHP says one thing, live HTML another — common when a deployment
...
Future readers should see: _"the rendered button literally says `Go to Homepage`, confirmed against the deployed app on 2026-05-15."_
...
The user wanted a test for the empty-state history page. The empirical loop:
...
3. **Verification path** — start with the PHP for the redirect logic (cheapest), then a quick live HTML inspection for the rendered strings.
...
- A new CURA flow / mechanism comes into scope.
...
- An **existing intentionally-failing test surprises you by passing.** Don't rewrite to "match the new reality" without going through the docs first
  (`IDENTIFIED_GAPS.md`, `CURA_FRD.md` §9). The pass may be the §A-ENV-1 transport flake making the gap test pass for the wrong reason; verify the
  _cause_ before changing the test.
...
- It does not flip intentionally-failing tests to green — the §9 gap tests and the BFcache reds are red by design and stay red.
...
- It does not surface probes themselves into the repo — probes live in a gitignored directory and are deleted once the finding lands in a test,
  comment, or `IDENTIFIED_GAPS.md` entry (per `CLAUDE.md` → "Throwaway probes").
