...
description:
  ...
  load-bearing state surfaces of this suite (Heroku dyno warm/cold, demo-account history persistence, leftover `.reports/` / `.ocarina_logs_*` /
  `.webdriver_profile_*` artifacts, browser-profile cleanliness, `--workers 3` concurrent access to a shared demo account, deployed-CURA vs
  github-source drift, chromedriver/geckodriver/browser version skew, recent driver or browser updates) and surfaces what to verify first. The
  environment is often "bouchonné" — half-configured, half-cached, half-warm — and a noisy failure that gets blamed on the test code is frequently a
  state artifact. Use whenever the user asks why a test misbehaves, why one run differs from another, why a flake appeared, why a fresh dispatch
  surprises, or before reaching for "the SUT must have changed". Cross-references `IDENTIFIED_GAPS.md` §A-ENV-* for the artifacts already catalogued.
---
...
SUT changed" or "the test is broken". This suite runs against a live demo deployed on a Heroku eco dyno, with a _shared_ demo account, with browser
profiles created on the fly, with reports / logs / screenshots accumulating on disk — and any one of those layers can be in a state that's quietly
biasing the outcome.
...
This is the empirical companion to `empiricism` and `write-a-probe`: those tools answer _what does CURA do?_; this one answers _what state is the run
in?_. Run it first when symptoms don't match the documented expected categories.
...
The deployed CURA app runs on a Heroku eco dyno that sleeps after inactivity. A cold dyno makes the first request slow; a `--workers 3` run can hit a
half-warm dyno and one worker gets a slow start. Cold-start cascades into false `TimeoutException`s (which auto-retry per `transient_errors`) and into
surprise greens on gap tests (the silent 2nd-POST drop, `IDENTIFIED_GAPS.md` §A-ENV-1).

Verification:

```bash
# is the dyno awake right now?
curl -sf --max-time 30 https://katalon-demo-cura.herokuapp.com/ -o /dev/null && echo "awake" || echo "cold or down"

# warm it explicitly before a local run
curl -sf --retry 6 --retry-delay 5 --retry-all-errors --max-time 30 https://katalon-demo-cura.herokuapp.com/ > /dev/null
```

The CI workflow already does this in a "Warm up Heroku dyno" step before `Run e2e cycle`. Local runs don't — flag if the local run was the first hit
after a long pause.

### 2. Demo account history persistence

CURA's `_SESSION['history']` is server-side, scoped to PHP session. **A logged-in user's history outlives one Python test process** for as long as the
PHPSESSID cookie is valid. Across `--workers 3`, three browser workers can simultaneously log in as `John Doe` — and they share the demo account's
recent history if the server stores it per username rather than per session (it stores per session, but session lifecycle is what to question).
...
If the history is non-empty for tests that assume empty, that's the cause. `rapid_logout_relogin` and `empty_history` both rely on a fresh-login
history being empty — `_f::logout()` does `session_unset()` + `session_destroy()` (per `IDENTIFIED_GAPS.md` §G-SEC-2), so a _new_ login starts fresh.
But state from a still-live cookie is a different story.
...
Chrome runs with the consumer password manager off (`create_drivers_pool.py` enforces this). If for any reason a probe or a local invocation bypasses
the adapter and launches plain Chrome, the post-login breach modal is back and _every_ downstream action looks like the test framework hanging.
`IDENTIFIED_GAPS.md` §A-ENV-2 documents the resolution; the artifact returns the moment the adapter isn't used.
...
Three browsers logging in as `John Doe` at the same time. CURA's `_f::login()` (per `IDENTIFIED_GAPS.md` §G-SEC-3) compares with `===` and has no rate
limit / lockout. So three simultaneous logins are _fine_ on CURA's side — but on the Heroku eco dyno, three simultaneous POSTs can produce the
`--workers 3` shared-dyno rapid-POST flake (§A-ENV-1).
...
### 6. Deployed CURA vs github-source drift
...
drift. CSRF is the canonical example (per `IDENTIFIED_GAPS.md` §G-SEC-1): the github source calls `$antiCSRF->insertHiddenToken()` before the
...
exactly the misdiagnosis trap.

Verification: read the rendered HTML of the deployed page, not the github source:
...
The Heroku eco dyno is shared globally — anyone hitting the demo at the same time uses the same dyno. If a flake appeared _only_ during a specific
window (e.g. a popular tutorial uses CURA and a class is following along), the dyno may be under unusual concurrent load that has nothing to do with
this suite.
...
### 3. Cross-reference `IDENTIFIED_GAPS.md` §A-ENV-\*

The two catalogued environmental artifacts:

- **§A-ENV-1** — `--workers 3` shared-dyno rapid-POST drop. Symptom: a rapid 2nd-in-a-row POST gets no confirmation, `TimeoutException` surfaces after
  auto-retry. Affects `Saturation`, `Journey - History ordered`, can false-negative the duplicate/overlapping gap tests.
- **§A-ENV-2** — Chrome password-breach modal (resolved). Symptom: any post-login interaction silently does nothing. The driver adapter
  (`create_drivers_pool.py`) disables the password manager so this is no longer hit through `python -m src.main` — but a probe that bypasses the
  adapter can resurrect it.
...
| -------------------------------- | ---------------------------- | ---------- |
| Heroku dyno warm/cold            | <Confirmed / Possible / Not> | <one line> |
| Demo account history persistence | …                            | …          |
| Leftover local artifacts         | …                            | …          |
| Browser profile cleanliness      | …                            | …          |
| `--workers 3` rapid POSTs        | …                            | …          |
| Deployed CURA vs source drift    | …                            | …          |
...
- It does not generalise environment artifacts that aren't catalogued. If you find a new one (a third browser-side oddity, a new dyno behaviour), file
  it in `IDENTIFIED_GAPS.md` §A-ENV-N first, then this audit references it on the next walk.
- It does not silence flakes. A confirmed `--workers 3` shared-dyno flake is handled by `transient_errors` retry already; if it survives the retry,
  it's a documented artifact, not a regression.
...
- Before invoking `update-frd-and-tests` to "mark a gap resolved" — the surprise green may be §A-ENV-1, not CURA.
