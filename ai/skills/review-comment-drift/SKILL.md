...
3. **Scenario top docstring** — the `Flow:` arrow chain, the `Pre-fragments:` / `Post-fragments:` lists, the one-paragraph summary. These are also
   **mechanically checkable** against the actual `drive_page` sequence and the adapter's `create_*_test` kwargs, so the audit can do them cheaply.
...
| **`Pre-fragments:` / `Post-fragments:` list doesn't match the kwarg**     | Scenario top docstring                                                                                                                                                        | Parse `create_*_test(... pre_test_scenarios_fragments=[...], post_test_scenarios_fragments=[...])` and compare to the docstring.    |
...
Parse the adapter's `create_*_test(...)` calls (`create_selenium_test` / `create_playwright_test`; one or more in a single file for data-driven). For
each, extract `pre_test_scenarios_fragments=[…]` and `post_test_scenarios_fragments=[…]`. Cross-check against the docstring's `Pre-fragments:` /
`Post-fragments:` lines.
