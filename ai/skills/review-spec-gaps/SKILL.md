...
**Default target:** `CURA_FRD.md`. Cross-reference `CURA_TEST_STRATEGY.md` and `IDENTIFIED_GAPS.md` to avoid surfacing questions that are already
documented elsewhere.

**This is a static review.** No probes. No live-app observation. No assertions about CURA's actual behaviour. The skill asks _"is this clear from the
document?"_ — not _"is the document right?"_.
...
Example question: _"REQ-AUTH-1 covers login with valid and invalid credentials. What happens when the username contains trailing whitespace? Is that a
separate rejection path or treated as the empty case?"_
...
Example question: _"REQ-APPT-2 says a visit date is required. The boundary isn't defined: is today a valid visit date, or is the minimum tomorrow? Is
there a maximum (a year out, a decade)?"_
...
Example question: _"REQ-APPT-3 (booking) describes the success path. If the submission fails, is the user shown a specific error, returned to the form
with values preserved, or sent elsewhere?"_
...
- Empty states (already covered by REQ-HIST-4 here, good).
- Hover, focus, error styling.

Example question: _"REQ-AUTH-1 says 'Login failed!' is displayed on bad credentials. Is the error displayed inline, as a toast, on a redirect? Is the
username preserved in the form after the failure?"_
...
Example question: _"REQ-HIST-3 says history renders most-recent-first. Does the spec define what history shows immediately after a fresh login (no
bookings yet, before any session activity)?"_ (Answer for this spec: yes — REQ-HIST-4. Don't flag.)
...
Example question: _"REQ-APPT-2 names `visit_date` as required. The validation strategy (client-side only? server-side? both?) isn't stated. The §9.1
gap entry tells us empirically that server-side validation is missing — should the spec define what the server **should** do, even if §9.1 reports
CURA doesn't?"_
...
Example question: _"REQ-AUTH-2 (session management) and REQ-APPT-3 (booking) both exist; how should the system behave if the session expires
mid-form-fill? Is this defined, or does it fall back to whatever the framework does?"_
...
Example question: _"REQ-AUTH-1 says authentication is 'case-sensitive'. The spec confirms this for the username and password. Is whitespace
sensitivity also defined? (The §5 coverage table tests `lowercase` and `uppercase` cases; whitespace doesn't appear.)"_
...
- Inferred element IDs (the FRD does document these well; less of a gap here).
- Inferred order of operations.
- Inferred default values.

Example question: _"The visit-date input uses `dd/mm/yyyy` per the §6 form table. Is the date format defined as a requirement, or inferred from the
input? If a user pastes `2026-05-15`, what does the system do?"_
...
### 2. Walk each REQ-\* and §9 entry against the nine categories

For each requirement section, ask the nine questions. Filter:

- **Specified** (silent) — the requirement covers this dimension.
- **Documented elsewhere** (silent) — the dimension is in `CURA_TEST_STRATEGY.md`, `IDENTIFIED_GAPS.md`, or a §9 entry. Don't double-surface.
...
Some questions are answered by the strategy doc rather than the FRD. If §1 explicitly scopes out a dimension ("performance is out of scope"), don't
flag adjacent performance questions.
...
```markdown
# Spec-clarity review — `CURA_FRD.md` (+ strategy cross-check)
...
- **REQ-APPT-2 — visit-date boundary.** The date is required; the upper / lower bound isn't defined. The §9.7 gap notes that past dates are currently
  accepted, which suggests the **intended** lower bound is "today or later", but the spec doesn't state it directly. _What is the intended minimum and
  maximum visit date?_
...
- (nothing open — REQ-HIST-1..4 cover the states well; the §9.8 ordering gap is documented)

### Profile

- **REQ-PROF-1 — content of the empty profile.** The §9.2 entry says the profile is a placeholder with no editable fields. _Is the placeholder
  intentional and stable (a documented empty surface), or a known incomplete feature that the FRD should call out as "not yet built"?_
...
- **Scope out** — explicitly mark it out of scope (in §1 of the strategy doc, or as a note in the FRD section).
...
- **CURA's actual defects.** Those go in `IDENTIFIED_GAPS.md` and FRD §9, identified by empirical observation (per `empiricism`). This skill is about
  _spec clarity_, not CURA behaviour.
...
- **Out-of-scope dimensions.** If the strategy's §1 lists "performance" / "accessibility" / "payment" as out of scope, don't ask performance /
  accessibility / payment questions.
...
- After a CURA spec change — the change may have introduced new ambiguities.
- Onboarding a contributor — a clarity review feeds back to the FRD.

## What this skill does NOT do

- It does not edit the FRD or the strategy doc.
- It does not run probes. (If a question really is "does CURA do X?", reach for `empiricism` / `write-a-probe` — different motion.)
