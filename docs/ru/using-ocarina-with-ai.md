...
Шаги онбординга (venv, `pip install`, набор скиллов, скопированный в Claude Code, `ruff` /&nbsp;`mypy` /&nbsp;`pre-commit`, smoke-проверка раннера)
описаны в `setup-environment`.
...
- `review-dead-code`&nbsp;—&nbsp;неиспользуемые connectors&nbsp;/&nbsp;POM /&nbsp;сценарии /&nbsp;suites /&nbsp;фрагменты /&nbsp;константы; для каждой
  находки: удалить, инкубировать (`<source-root>/incubator/`, дерево зависимостей сохранено) или оставить.
...
**Набор не зелёный:** `review-report`&nbsp;→&nbsp;`analyse-*` →&nbsp;`write-a-probe` →&nbsp;находка ложится в `IDENTIFIED_GAPS.md` /&nbsp;FRD
/&nbsp;комментарий сценария&nbsp;→&nbsp;probe удаляется.
